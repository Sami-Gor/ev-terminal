...
import { $, fnum, fchg, fpct, fpctOr, arrowOr, clsOr } from '../utils/format.js';
...
export function renderSector() {
  ...
  for (const key of Object.keys(SECTORS)) {
    const priced = universe.filter(t => t.sector === key && Number.isFinite(t.pct));
    const avg = new Array(24).fill(0);
    priced.forEach(t => {
      intradayPath(t.pct, t.sym + '|i', rngFor).forEach((v, j) => { avg[j] += v / priced.length; });
    });
    series[key] = priced.length ? avg : null;
  }
  ...
  for (const key of Object.keys(SECTORS)) {
    if (!series[key]) continue;                     // no priced members yet — no fabricated line
  ...
  $('sect-legend').innerHTML = Object.keys(SECTORS).map(key => {
    const v = series[key] ? series[key][23] : null;
    return `<span><span class="sw" style="background:${key === 'PURE' ? upColor() : SECTORS[key].color}"></span>${SECTORS[key].label} <b class="${clsOr(v)}">${fpctOr(v)}</b></span>`;
  }).join('');
  $('sect-table').innerHTML = '<tr><th>SECTOR</th><th>AVG CHG</th><th>n</th><th>WEIGHTING</th><th>AS-OF</th></tr>' +
    Object.keys(SECTORS).map(key => {
      const v = series[key] ? series[key][23] : null;
      return `<tr><td style="color:${key === 'PURE' ? upColor() : SECTORS[key].color}">${SECTORS[key].label}</td><td class="${clsOr(v)}">${arrowOr(v)} ${fpctOr(v)}</td><td>${universe.filter(t => t.sector === key).length}</td><td>equal-weight</td><td>16:05 ET</td></tr>`;
    }).join('');
}
