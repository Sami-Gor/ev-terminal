...
export function initTrackers() {
  ...
  $('tk-btn').addEventListener('click', e => {
    e.stopPropagation();
    ['cards-pop', 'alert-pop', 'up-pop'].forEach(id => { $(id).classList.remove('open'); $(id.replace('tk-pop','tk-btn'))?.setAttribute('aria-expanded','false'); });
  ...
  ['p-heat', 'p-tick'].forEach(id => {           // ± shortcut buttons
    ...
    tools.querySelector('[data-act="tk"]').addEventListener('click', e => {
      e.stopPropagation();
      ['cards-pop', 'alert-pop', 'up-pop'].forEach(pid => $(pid).classList.remove('open'));
      $('tk-pop').classList.toggle('open');
    });
  });
